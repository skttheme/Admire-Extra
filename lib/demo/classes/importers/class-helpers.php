...
class FWP_Demos_Helpers {
	...
	public static function wp_get_http( $url, $file_path = false, $red = 1 ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $body, FS_CHMOD_FILE ) ) {
		    // If the attempt to write to the file failed, delete the file (if it exists).
		    wp_delete_file( $file_path );

		    // Fallback to another WP_Filesystem method if necessary.
		    // For example, you can try writing the file again or log the error.
		    if ( ! $wp_filesystem->put_contents( $file_path, $body, FS_CHMOD_FILE ) ) {
		        return false; // Return false if the file still cannot be written.
		    }
		}
